...
constructRASLAMDataSubmatrices(const RelativeMeasurements &measurements) {
  ...
  getGraphDimensionsFromLocalMeasurements(measurements, &d, &n, &l, &b);
  const size_t mMeasurements = measurements.vec.size();
  ...
  CHECK_EQ(mPose + mRange, mMeasurements);
  CHECK_EQ(mRange, l);

  /**
   * @brief CORA vs DCORA implementation.
   * - In DCORA, we populate the transpose of the submatrices, where as CORA
   *   populates the submatrices themselves.
   * - DCORA compresses its sparse matrices, whereas CORA does not. We
   *   empirically observe compression does not affect the results. With or
   *   without compression, the entries of DCORA and CORA data matrices are off
   *   by no more than 1e-7.
   */

  /**
   * @brief Constructing submatrices of data matrix Q_p
   *
   * Data matrix Q_p is a symmetric block matrix of the form:
   *
   *  <-----------col------------>
   *
   *      dn       l      n + b
   ...
   *           = ARho^T * OmegaRho * ARho +
   *             T^T * OmegaTau * T
   *           = ARhoT * OmegaRho * ARhoT.transpose() +
   *             TT * OmegaTau * TT.transpose()
   *
   *    Q_p_13 = V
   *           = T^T * OmegaTau * ATau
   *           = TT * OmegaTau * ATauT.transpose()
   *
   *    Q_p_33 = L(G^tau)
   *           = ATau^T * OmegaTau * ATau
   *           = ATauT * OmegaTau * ATauT.transpose()
   *
  ...
  const size_t rowsARhoT = d * n;
  const size_t colsARhoT = d * mPosePose;
  const size_t rowsATauT = n + b;
  const size_t colsATauT = mPose;
  const size_t rowsTT = d * n;
  ...
  for (size_t k = 0; k < mPosePose; k++) {
    ...
    for (size_t c = 0; c < d; c++)
      for (size_t r = 0; r < d; r++)
        ARhoT.insert(i * d + r, k * d + c) = -meas.R(r, c);

    /// Assign d-identity matrix to block leaving node j
    // AT(j,k) = +I (NOTE: POSITIVE)
    for (size_t r = 0; r < d; r++)
      ARhoT.insert(j * d + r, k * d + r) = +1;

    // Assign isotropic weights in diagonal matrix
    for (size_t r = 0; r < d; r++)
      diagonalOmegaRho[k * d + r] = meas.weight * meas.kappa;
  }

  // Populate ATauT, OmegaTauT, and TT
  for (size_t k = 0; k < mPosePose; k++) {
    const RelativePosePoseMeasurement &meas = pose_pose_measurements.at(k);
    CHECK_EQ(meas.r1, meas.r2);
    size_t i = meas.p1;
  ...
  for (size_t k = mPosePose; k < mPose; k++) {
    ...
    CHECK_EQ(meas.r1, meas.r2);
    size_t i = meas.p1;     // No offset for pose indices
    size_t j = meas.p2 + n; // Offset landmark indices by the number of poses
  ...
  /**
   * @brief Constructing submatrices of data matrix Q_r
   *
   * Data matrix Q_r is a symmetric block matrix of the form:
   *
   *  <-----------col------------>
   *
   *       dn      l      n + b
   ...
   *
   *  where:
   *    Q_p_22 = OmegaRange * D^2
   *           = OmegaRange * DT.transpose() * DT.transpose()
   *
   *    Q_p_23 = D * OmegaRange * C
   *           = DT.transpose() * OmegaRange * CT.transpose()
   *
   *    Q_p_33 = C^T * OmegaRange * C
   *           = CT * OmegaRange * CT.transpose()
   *
   *  Dimensions: [rows x cols]
   *    Incidence Matrices:
   *      CT: [(n + b) x l] diagonal matrix
   *    Weight Matrices:
   *      OmegaRange: [l x l] diagonal matrix
   *    Data Matrices:
   *      DT: [l x l] matrix
   *
   *    Note: OmegaRangeT = OmegaRange, mRange = l
   */

  // Define matrix dimensions
  const size_t rowsCT = n + b;
  const size_t colsCT = l;
  const size_t rowsDT = l;
  const size_t colsDT = l;
  ...
  // Populate submatrices and dimensions
  RASLAMDataSubmatrices dataSubmatrices;
  dataSubmatrices.ARho = ARhoT.transpose();
  dataSubmatrices.ATau = ATauT.transpose();
  dataSubmatrices.T = TT.transpose();
  dataSubmatrices.OmegaRho = OmegaRho;
  dataSubmatrices.OmegaTau = OmegaTau;
  dataSubmatrices.C = CT.transpose();
  dataSubmatrices.D = DT.transpose();
  dataSubmatrices.OmegaRange = OmegaRange;
  dataSubmatrices.d = d;
  dataSubmatrices.n = n;
  dataSubmatrices.l = l;
  dataSubmatrices.b = b;

  return dataSubmatrices;
}
...
constructRASLAMDataMatrix(const RelativeMeasurements &measurements) {
  ...
  RASLAMDataSubmatrices dataSubmatrices =
      constructRASLAMDataSubmatrices(measurements);

  // Get variable references from dataSubmatrices
  const SparseMatrix &ARho = dataSubmatrices.ARho;
  const SparseMatrix &ATau = dataSubmatrices.ATau;
  const DiagonalMatrix &OmegaRho = dataSubmatrices.OmegaRho;
  const DiagonalMatrix &OmegaTau = dataSubmatrices.OmegaTau;
  const SparseMatrix &T = dataSubmatrices.T;
  const SparseMatrix &C = dataSubmatrices.C;
  const DiagonalMatrix &OmegaRange = dataSubmatrices.OmegaRange;
  const SparseMatrix &D = dataSubmatrices.D;
  const unsigned int d = dataSubmatrices.d;
  const unsigned int n = dataSubmatrices.n;
  const unsigned int l = dataSubmatrices.l;
  const unsigned int b = dataSubmatrices.b;

  // Calculate Q block matrices
  // clang-format off
  SparseMatrix Q11 = ARho.transpose() * OmegaRho * ARho +
                     T.transpose() * OmegaTau * T;
  SparseMatrix Q13 = T.transpose() * OmegaTau * ATau;
  SparseMatrix Q22 = OmegaRange * D * D;
  SparseMatrix Q23 = D * OmegaRange * C;
  SparseMatrix Q33 = ATau.transpose() * OmegaTau * ATau +
                     C.transpose() * OmegaRange * C;
  // clang-format on

  /**
   * @brief Construct data matrix Q
   *
   * The following implementation is adapted from:
   * CORA: https://github.com/MarineRoboticsGroup/cora
   */
  ...
  const size_t rotMatSize = d * n;
  const size_t rotRangeMatSize = rotMatSize + l;
  const size_t dataMatSize = rotRangeMatSize + n + b;
  ...
  SparseMatrix dataMatrix(dataMatSize, dataMatSize);
  dataMatrix.setFromTriplets(combinedTriplets.begin(), combinedTriplets.end());
